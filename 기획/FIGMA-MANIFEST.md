...
> 📁 **폴더 위치 정정 (2026-05-30)**: 풀스크린 4장을 `기획/02 메인/v0.2/큐레이션/` 서브 폴더에 통합 정리. 메인 도메인 폴더 안에서 모두 볼 수 있게. 기존 `기획/08 큐레이션/v0.4/05~08*.html`은 이력 보존. **임시 위치 — 다음 라운드에 별도 도메인 폴더로 분리할지 재검토.**
